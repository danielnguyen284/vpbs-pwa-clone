  ...
  try {
    ...
    if (ohlc) {
      ...
      if (price <= ohlc.h && price >= ohlc.l) {
        ...
        return NextResponse.json({ error: `Giá đặt (${price.toLocaleString('vi-VN')}) nằm ngoài biên độ giao dịch của ngày này (${ohlc.l.toLocaleString('vi-VN')} - ${ohlc.h.toLocaleString('vi-VN')} VND).` }, { status: 400 });
      }
    } else {
      return NextResponse.json({ error: 'Không tìm thấy dữ liệu lịch sử giá giao dịch cho ngày này.' }, { status: 400 });
    }

    let portfolioDoc = await Portfolio.findOne({ userId: user._id, symbol });

    if (side === 'M') { // BUY
      if (user.cash_balance < costAfterFee) {
        return NextResponse.json({ error: 'Sức mua không đủ (Không đủ trả phí 0.15%)' }, { status: 400 });
      }
      ...
      if (isMatched) {
        ...
        if (portfolioDoc) {
          ...
          const totalCost = (portfolioDoc.avg_price * portfolioDoc.total_qty * 1000) + costAfterFee;
      ...
      if (!portfolioDoc) {
        return NextResponse.json({ error: 'Không có cổ phiếu này trong danh mục' }, { status: 400 });
      }

      // Calculate available_qty for sell day (diffDays >= 2)
      const orderTsUTC = Date.UTC(requestedDate.getUTCFullYear(), requestedDate.getUTCMonth(), requestedDate.getUTCDate());
      let availableQty = 0;
      let availableLots: { lot: any, index: number }[] = [];

      portfolioDoc.lots.forEach((lot: any, idx: number) => {
        const lotTsUTC = Date.UTC(lot.date.getUTCFullYear(), lot.date.getUTCMonth(), lot.date.getUTCDate());
        const diffDays = Math.floor((orderTsUTC - lotTsUTC) / (1000 * 3600 * 24));
        if (diffDays >= 2) {
      ...
      if (availableQty < quantity) {
        return NextResponse.json({ error: `Số lượng khả dụng không đủ (K.Dụng: ${availableQty}, Cần bán: ${quantity})` }, { status: 400 });
      }
      ...
      if (isMatched) {
        ...
        availableLots.sort((a, b) => new Date(a.lot.date).getTime() - new Date(b.lot.date).getTime());
    ...
    const order = await Order.create({
      userId: user._id,
      symbol, side, price, quantity, order_date: requestedDate, status, filled_price: filledPrice, fee: isMatched ? fee : 0
    });
