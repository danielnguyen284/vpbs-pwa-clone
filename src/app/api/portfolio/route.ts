...
export async function GET() {
  try {
    ...
    const enriched = await Promise.all(portfolios.map(async (p: any) => {
      ...
      let t0_qty = 0;
      let t1_qty = 0;
      let t2_qty = 0; // Everything >= 2 is available directly, but returning 0 for UI

      if (p.lots) {
        p.lots.forEach((lot: any) => {
          const lotDate = new Date(lot.date);
          const lotUTC = Date.UTC(lotDate.getUTCFullYear(), lotDate.getUTCMonth(), lotDate.getUTCDate());
          const diffDays = Math.floor((todayUTC - lotUTC) / (1000 * 3600 * 24));
